...
{
	if (in == CorrectAnswer) return mxDependValue;

	if (in == PartlyCorrect && mxScore == 0) return mxDependValue;
...
inline int statusToScore(int ratio, int mxScore)
{
	return 1ll * mxScore * ratio / mxDependValue;
}

inline QString statusRankingText(int ratio)
{
	if (ratio >= mxDependValue) return QObject::tr("Pure");

	if (ratio < 0) return QObject::tr("Lost");
