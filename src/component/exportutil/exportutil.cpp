...
auto ExportUtil::getContestantHtmlCode(Contest *contest, Contestant *contestant, int num) -> QString {
	...
	for (int i = 0; i < taskList.size(); i++) {
		...
		htmlCode +=
		    QString(
		        R"(<th scope="col" class="a">%1</th></tr>)")
		        .arg(tr("Score"));
			...
			for (int k = 0; k < inputFiles[j].size(); k++) {
				...
				if (k == 0) {
					if (score[j].size() == inputFiles[j].size())
						htmlCode +=
						    QString(
						        R"(<td rowspan="%1" class="a">#%2</td>)")
						        .arg(inputFiles[j].size())
						        .arg(j + 1);
					else
						htmlCode +=
						    QString(
						        R"(<td rowspan="%1" class="a">#%2<br>%3:%4</td>)")
						        .arg(inputFiles[j].size())
						        .arg(j + 1)
				...
				htmlCode += QString("<td class=\"a\">%1</td>")
				                .arg(inputFiles[j][k]);
				QString text;
...
void ExportUtil::exportHtml(QWidget *widget, Contest *contest, const QString &fileName) {
	...
	out << QString(
	           R"(<th class="b" scope="col">%1</th>)")
	           .arg(tr("Rank"));
	out << QString(
	           R"(<th class="b" scope="col">%1</th>)")
	           .arg(tr("Name"));
	out << QString(
	           R"(<th class="g" scope="col">%1</th>)")
	           .arg(tr("Total Score"));
	...
	for (auto &i : sortList) {
		...
		out << QString("<tr><td class=\"a\">%1</td>")
		...
		for (int j = 0; j < inputFiles.size(); j++) {
			for (int k = 0; k < inputFiles[j].size(); k++) {
				...
				if (k == 0) {
					...
					htmlCode +=
					    QString(
					        R"(<td rowspan="%1"><a class="c">%2</a> / %3</td>)")
					        .arg(inputFiles[j].size())
					        .arg(minv)
					        .arg(maxv);
				}
